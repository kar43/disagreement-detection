...
def prepare_bert_embeddings(input_df, save1, save2):
	...
	tokenized1 = input_df["title1_en"].apply((lambda x: tokenizer.encode(x[:510], add_special_tokens=True)))
	tokenized2 = input_df["title2_en"].apply((lambda x: tokenizer.encode(x[:510], add_special_tokens=True)))
...
train_df = pd.read_csv('../ByteDance_Dataset/train_bd_mt2nd.csv')
...
prepare_bert_embeddings(train_df, "bd/pre_bert_bd", "bd/hyp_bert_bd")
prepare_bert_embeddings(test_df, "bd/pre_bert_test_bd", "bd/hyp_bert_test_bd")